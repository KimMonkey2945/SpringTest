...
<body>
	...
	<h1>멤버십</h1>
		<table class="table text-center">
			...
			<c:forEach var="members" items="${membership }" varStatus="status">
				...
				<tr>
					...
					<c:choose>
						...
						<c:otherwise><td>${members.grade }</td></c:otherwise>
					</c:choose>
					
					<c:if test="${members.point >= 5000 }">
						<td class="text-primary">${members.point }</td>
					</c:if>
					<c:if test="${members.point <= 5000 }">
						<td>${members.point }</td>
					</c:if>
